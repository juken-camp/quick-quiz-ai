    ...
    sh.forEach((txt, i) => {
        ...
        b.onclick = () => {
            ...
            if (!answered) {
                ...
                // 不正解時：AIによる間違い分析を自動チャット送信
                if (!isCorrect && aiMode.autoAnalyze) {
                    setTimeout(() => {
                        const autoMsg = `「${q.q}」という問題で、「${txt}」と答えたけど間違えたよ。正解は「${correctText}」だったんだ。なぜ間違えやすいの？どう覚えればいい？`;
                        sendChat(autoMsg, true);
                        // チャットパネルを自動で開く
                        document.getElementById('chatPn').classList.add('show');
                        document.getElementById('chatBd').classList.add('show');
                    }, 500);
                }
            }
            if (i === ci) {
                mg.innerHTML = '🎉 正解！ ' + (q.exp || '');
                rs.className = 'quiz-res ok';
            } else {
                mg.innerHTML = '❌ 不正解。正解は「' + correctText + '」 ' + (q.exp || '');
...
async function sendChat(message, isAuto = false) {
    ...
    // User bubble（自動送信は「AI分析中…」ラベルを付ける）
    const ub = document.createElement('div');
    ub.className = 'chat-m u';
    ub.textContent = isAuto ? '🤖 間違い分析してもらうね…' : message;
    box.appendChild(ub);
    if (!isAuto) { inp.value = ''; }
    btn.disabled = true;
    ...
    if (!document.getElementById('chatPn').classList.contains('show')) {
        document.getElementById('chatPn').classList.add('show');
        document.getElementById('chatBd').classList.add('show');
    }
